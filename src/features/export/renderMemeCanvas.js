...
export async function renderMemeCanvas(result) {
  ...
  const image = await loadImageWithFallback(result.imageUrl, result.scene);
  ...
  if (result.promptLabel) {
    drawPill(ctx, result.promptLabel, 430, 880, "#ffffff", "#24201a");
  }
  ...
  wrapText(ctx, result.summary || result.text, 130, 1210, 820, 44, 2);

  ctx.fillStyle = "#24201a";
  ctx.font = "700 32px system-ui, sans-serif";
  ctx.fillText(APP_NAME, 130, 1295);
...
async function loadImageWithFallback(src, scene) {
  try {
    return await loadImage(src);
  } catch {
    return loadImage(getPlaceholderImage(scene));
  }
}
